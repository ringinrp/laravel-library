...
export default function Edit(props) {
    ...
    const onHandleSubmit = (e) => {
        e.preventDefault();
        put(route('admin.book-stock-reports.update', props.stock.id), {
            preserveScroll: true,
            preserveState: true,
            onSuccess: (success) => {
    ...
    return (
        <div className="flex w-full flex-col pb-32">
            <div className="gap-y-4 mb-8 flex flex-col items-start justify-between lg:flex-row lg:items-center">
                <HeaderTitle
                ...
                <CardContent className="p-6">
                    <form className="space-y-6" onSubmit={onHandleSubmit}>
                        ...
                        <div className="grid w-full items-center gap-1.5">
                            ...
                            <Input name="available" id="available" type="number" value={data.available} disabled />
                            {errors.available && <InputError message={errors.available} />}
                        </div>
                        <div className="grid w-full items-center gap-1.5">
                            <Label htmlFor="loan">Dipinjam</Label>
                            <Input name="loan" id="loan" type="number" value={data.loan} disabled />
                            {errors.loan && <InputError message={errors.loan} />}
                        </div>
                        <div className="grid w-full items-center gap-1.5">
                            <Label htmlFor="lost">Hilang</Label>
                            <Input name="lost" id="lost" type="number" value={data.lost} disabled />
                            {errors.lost && <InputError message={errors.lost} />}
                        </div>
                        <div className="grid w-full items-center gap-1.5">
                            <Label htmlFor="damaged">Rusak</Label>
                            <Input name="damaged" id="damaged" type="number" value={data.damaged} disabled />
