...
class FineReportController extends Controller
{
    ...
    {
        $fines = Fine::query()
            ->select(['id', 'return_book_id', 'user_id', 'late_fee', 'other_fee', 'total_fee', 'fine_date', 'status', 'created_at'])
        ...
        return inertia('Admin/FineReports/Index', [
            ...
            'page_data' => [
                ...
                'fine_paid' => Fine::query()
                    ->where('status', FinePaymentStatus::SUCCESS->value)
                    ->sum('total_fee'),
                'fine_pending' => Fine::query()
                    ->whereNot('status', FinePaymentStatus::SUCCESS->value)
                    ->sum('total_fee'),
